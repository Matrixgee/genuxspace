...
const AdminWithdraw = () => {
  ...
  const fetchWithdrawals = async () => {
    try {
      const response = await axios.get(
        "https://gen-75n5.onrender.com/api/admin/getWithdrawals",
        {
          headers: {
  ...
  const handleApproveWithdraw = async (_id: string) => {
    ...
    try {
      const response = await axios.put(
        `https://gen-75n5.onrender.com/api/admin/approveWithdrawal/${_id}`,
        { status: "approved" },
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  const handleDeclineWithdraw = async (_id: string) => {
    ...
    try {
      const response = await axios.put(
        `https://gen-75n5.onrender.com/api/admin/declineWithdrawal/${_id}`,
        { status: "rejected" },
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  const handleProcessingWithdraw = async (_id: string) => {
    ...
    try {
      const response = await axios.put(
        `https://gen-75n5.onrender.com/api/admin/pendingWithdrawal/${_id}`,
        { status: "processing" },
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
