...
const AdminDeposit = () => {
  ...
  const confirmDeposit = async (transactionId: string) => {
    const confirmUrl = `https://gen-75n5.onrender.com/api/admin/approveDeposit/${transactionId}`;
    try {
      await axios.put(
        confirmUrl,
        {},
  ...
  const handleDecline = async (transactionId: string) => {
    const declineUrl = `https://gen-75n5.onrender.com/api/admin/declineDeposit/${transactionId}`;
    try {
      await axios.put(
        declineUrl,
        {},
